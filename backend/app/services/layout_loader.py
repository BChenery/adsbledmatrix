...
async def load_layouts_for_config(
    ...
    config: UserConfig,
) -> tuple[Optional[Any], Optional[Any], Optional[Any], List[Any], bool]:
    """Return (active, idle, focus, playlist, rotation_enabled) for a config row."""
    active = None
    idle = None
    focus = None
    playlist: List[Any] = []
    ...
    return active, idle, focus, playlist, bool(config.layout_rotation_enabled)
...
async def apply_engine_layouts(config: UserConfig, session: Optional[AsyncSession] = None) -> None:
    ...
    if session is not None:
        active, idle, focus, playlist, rotation = await load_layouts_for_config(session, config)
        engine.set_layout(
            active,
            idle,
            focus_layout=focus,
    ...
    async with AsyncSessionLocal() as refresh_session:
        active, idle, focus, playlist, rotation = await load_layouts_for_config(
